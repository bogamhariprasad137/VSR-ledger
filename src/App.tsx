...
export default function App() {
  ...
  useEffect(() => {
    ...
    const verifyAndFetch = async () => {
      try {
        let session = null;
        const isSupabaseAvailable = !!(import.meta as any).env?.VITE_SUPABASE_URL;
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (isSupabaseAvailable) {
        ...
        setAuthError("Password reset is not available in offline/local mode. For admin reset, please contact the developer.");
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isSupabaseAvailable || email.trim() === "admin") {
        // Authenticate with the local Express session-backed auth
        ...
        if (res.ok && resData.success) {
          ...
          errorMsg = resData.error || "Invalid local credentials. For local mode, try username 'admin'.";
        }
      } else {
        ...
        if (error) {
          ...
          errorMsg = "Failed to initiate secure admin session.";
        }
      }
